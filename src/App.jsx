 ...
 function App() {
  ...
  return (
    <>
      ...
      <div className="container mx-auto max-w-2xl my-10 rounded-xl p-6 bg-violet-100 dark:bg-gray-900 shadow-lg min-h-[80vh] transition-colors duration-300">
        ...
        <div className="todos space-y-3">
          {todos.length===0 && <p className='text-gray-500'>no todos available please add some todos</p>}
            {todos.map(item => {

                 
         return  <div key={item.id} className="todo flex justify-between items-center bg-white dark:bg-gray-700 shadow-md rounded-md p-3">
            <input name={item.id} onChange={handleCheckbox} type="checkbox" value={item.isCompleted}  id='' />
            <div className= '{item.isCompleted?"": "line-through"} text-gray-800 dark:text-gray-100'>{item.todo}</div>
            <div className="flex gap-2">
              <button onClick={handleEdit} className="bg-yellow-500 hover:bg-yellow-600 px-3 py-1 text-sm font-medium text-white rounded-md transition-all duration-200">
                Edit
              </button>
